# -*- coding: utf-8 -*-
"""
Created on Mon May 19 10:55:27 2025
...
# Paths and constants
base_path = 'G:/export'
output_dir = 'C:/Users/ruffy/OneDrive/result'
...
#%% Step 1–3 : load data → feature-selection
print("Step 1–3 : loading data and selecting good channels …")
...
# ── 1)  load files and build channel_data ────────────────────────────
        ...
        for idx in range(body_hg.shape[0]):
            ...
            for tr in range(body_hg.shape[-1]):
                ...
                if x.shape[0] == 55 and not np.any(np.isnan(x)) and not np.all(x == 0):
                    channel_data[ba][real_ch].append((x, 1))
            for tr in range(nonbody_hg.shape[-1]):
                x = nonbody_hg[idx, :, tr]
                if x.shape[0] == 55 and not np.any(np.isnan(x)) and not np.all(x == 0):
                    channel_data[ba][real_ch].append((x, 0))

    except Exception as e:
        print(f"Error loading {subj}: {e}")

# ── 2)  feature-selection (store mapping only if channel survives) ──
for ba, chan_dict in channel_data.items():
    for ch, trials in chan_dict.items():
        ...
        for start in window_starts:
            X_win = X_train[:, start:start + win_size]
            cv = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=42)
            fold = []
            for tr, te in cv.split(X_win, y_train):
                mdl = make_pipeline(StandardScaler(), SVC(kernel='linear'))
                mdl.fit(X_win[tr], y_train[tr])
                fold.append(mdl.score(X_win[te], y_train[te]))
        ...
        if np.mean(accs) > 0.5:
            ...
            # record once; duplicates cannot occur because each (ba,ch) appears once here
            selected_channels_by_subj[ba].append((subj_folder, ch))

# after the for-loops end, add:
selected_channels_by_subject = selected_channels_by_subj


print("✔ Step 1–3 done – channels kept with accurate subject mapping.")

#%% Step 4: Grouped Multi-channel Decoding per Lobe

print("Step 4: Computing and plotting grouped decoding accuracy...")

# --- (A) Compute grouped decoding results for all BAs and save ---
...
for ba in selected_channels:
    ...
    ste = np.array(stds) / np.sqrt(n_folds)
    for t, acc, err in zip(window_centers, accs, ste):
        all_result_rows.append([ba, t, acc, err])
...
#%% Step 10 : write Sem_<time>.csv  (only at each BA's own FDR-significant peak)
...
# build one CSV per peak time (50 / 100 / …) ----------------------------
for t in sorted(set(ba_peak_time.values())):
    ...
    for ba, peak_t in ba_peak_time.items():
        ...
        for subj_folder, ch in selected_channels_by_subject.get(ba, []):
            ...
            if subj_num is None:   # skip unknown code
                continue
...
planned_pairs = [
    ...
    if ba_to_lobe[src] != ba_to_lobe[tgt]        
    and ba_peak_times[src] < ba_peak_times[tgt]  
]
...
category_names = {0: "Non-body part", 1: "Body part"}

for src, tgt in planned_pairs:   
    # basic bookkeeping
    src_lobe = ba_to_lobe[src]
    tgt_lobe = ba_to_lobe[tgt]

    # concatenate trials (body + non-body) for each ROI
    X_src = np.concatenate([roi_trimmed[src][0], roi_trimmed[src][1]])
    Y_tgt = np.concatenate([roi_trimmed[tgt][0], roi_trimmed[tgt][1]])
    ...
    for t1 in range(usable_timepoints):
        for t2 in range(usable_timepoints):
            ...
            null_scores = [
                LinearRegression().fit(x, shuffle(y)).score(x, shuffle(y))
                for _ in range(n_permutations)
            ]
            pval_matrix[t1, t2] = np.mean(np.array(null_scores) >= r2_obs)

    # file-friendly BA names
    ba_src = src.split(" (")[0].replace(" ", "")
    ba_tgt = tgt.split(" (")[0].replace(" ", "")

    # save CSVs
    pd.DataFrame(r2_matrix).to_csv(
        os.path.join(output_dir, f"R2_CrossTemporal_{ba_src}_to_{ba_tgt}_Combined.csv"),
        index=False,
    )
    pd.DataFrame(pval_matrix).to_csv(
        os.path.join(output_dir, f"Pval_CrossTemporal_{ba_src}_to_{ba_tgt}_Combined.csv"),
        index=False,
    )
    ...
    xticks = np.arange(0, usable_timepoints, 10)
    plt.xticks(ticks=xticks, labels=[str(x * 10) for x in xticks])
    plt.yticks(ticks=xticks, labels=[str(x * 10) for x in xticks])
    plt.title(f"R²: {ba_src} → {ba_tgt}  |  Combined trials")
    plt.xlabel("Target time (ms)")
    plt.ylabel("Source time (ms)")
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"CrossTemporal_{ba_src}_to_{ba_tgt}_Combined.png")
    )
    plt.close()

print("✔  Step 12 complete – R² matrices & plots written only for planned pairs.")
